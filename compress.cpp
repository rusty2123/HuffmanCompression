...
int main()
{
    ...
    //discover the quickest way to create a table
    ...
    /***********************************************/
    //loops through the user inputted file name. 
    for (int i = 0; i < (int)strlen(fName) + 1; i++)
    {
        if (fName[i] == '.' || fName[i] == '\0')
        {
            strncpy_s(hufName, fName, i);
            strcat_s(hufName, ".huf");
            break;
        }
    }
    ...
    //loop through input file to find frequencies of symbols
    char symbol;
    bool found = false;
    ...
    do
    {
        ...
        if (found == false)
        {
            for (int j = 0; j < 256; j++)
            {
                if (frequencies[j].g == -1)
                {
                    frequencies[j].g = symbol;
                    ++frequencies[j].f;
                    ++glyphCount;
                    break;
                }
